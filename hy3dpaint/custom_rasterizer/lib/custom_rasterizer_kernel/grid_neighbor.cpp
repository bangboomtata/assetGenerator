    ...
    std::vector<torch::Tensor> view_layer_normals, int num_level, int resolution)
{
    ...
    texture_positions[0] = torch::zeros({static_cast<long>(seq2pos.size() / 3), 3}, float_options);
    texture_positions[1] = torch::zeros({static_cast<long>(seq2pos.size() / 3)}, float_options);
    ...
    for (int i = 0; i < grids.size(); ++i) {
        grid_neighbors[i] = torch::zeros({static_cast<long>(grids[i].seq2grid.size()), 9}, int64_options);
        long* nptr = grid_neighbors[i].data_ptr<long>();
        for (int j = 0; j < grids[i].seq2neighbor.size(); ++j) {
            nptr[j] = grids[i].seq2neighbor[j];
        }

        grid_evencorners[i] = torch::zeros({static_cast<long>(grids[i].seq2grid.size())}, int64_options);
        grid_oddcorners[i] = torch::zeros({static_cast<long>(grids[i].seq2grid.size())}, int64_options);
        ...
        if (i + 1 < grids.size()) {
            grid_downsamples[i] = torch::zeros({static_cast<long>(grids[i].downsample_seq.size())}, int64_options);
    ...
    int num_level, int resolution)
{
    ...
    texture_positions[0] = torch::zeros({static_cast<long>(seq2pos.size() / 3), 3}, float_options);
    texture_positions[1] = torch::zeros({static_cast<long>(seq2pos.size() / 3)}, float_options);
    texture_feats[0] = torch::zeros({static_cast<long>(seq2feat.size() / feat_channel), feat_channel}, float_options);
    float* positions_out_ptr = texture_positions[0].data_ptr<float>();
    memcpy(positions_out_ptr, seq2pos.data(), sizeof(float) * seq2pos.size());
    positions_out_ptr = texture_positions[1].data_ptr<float>();
    for (int i = 0; i < grids[0].seq2grid.size(); ++i) {
    ...
    for (int i = 0; i < grids.size(); ++i) {
        ...
        grid_evencorners[i] = torch::zeros({static_cast<long>(grids[i].seq2evencorner.size())}, int64_options);
        grid_oddcorners[i] = torch::zeros({static_cast<long>(grids[i].seq2oddcorner.size())}, int64_options);
        long* dptr = grid_evencorners[i].data_ptr<long>();
        for (int j = 0; j < grids[i].seq2evencorner.size(); ++j) {
            dptr[j] = grids[i].seq2evencorner[j];
        }
